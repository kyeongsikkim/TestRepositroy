...
public class Exam12DaoImpl3 implements Exam12Dao {
	...
	public int boardInsert(Exam12Board board) {
		int rows=sqlSessionTemplate.insert("board.insert", board);//insert하고 나서 board에 bno 가 들어가게 된다.
		//int rows 은 0아니면 1 이다 insert 삽입된 행위수 삽입 되던가 오류나던가.
		return board.getBno();
	}

	@Override
	public List<Exam12Board> boardSelectAll() {
	 List<Exam12Board> list=sqlSessionTemplate.selectList("board.selectAll");//네임스페이스.id 이다.
		return list;
	}
	...
	public void boardUpdate(Exam12Board board) {
		sqlSessionTemplate.selectOne("board.update",board);
	}	
